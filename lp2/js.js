...
$(document).ready(function () {
    ...
    if (desktop) {
        ...
        audioplay.setAttribute('src', '//media.securesaferoute.com/land2/common/sb_tmr_l/sound/msg.mp3');
    }

    var startedSignup = false;
    function playSound() {
        if (desktop) {
            audioplay.play();
        }
    ...
    var timerId = setInterval(function () {
        ...
        if (countdown <= 0) {
            ...
            $timer.html(min + ":" + "0" + sec).addClass("timesup");
        } else {
            $timer.html(min + ":" + sec);
        }
    }, 1000);
    var messageNumber = 0;
    var messagesTiming = 3500;
    var messageInterval;
    var $messageBlock = $(".chat-inner");
    var $chatItem = $(".chat-item");
    var messageLenght = $chatItem.length;

    function sendMessages() {
        function clearMessagesInterval() {
            clearInterval(messageInterval);
        }

        playSound();
        $('.typing-animation').addClass('hidden');
        if (messageNumber <= messageLenght) {
            setTimeout(function () {
                $chatItem.eq(messageNumber).show();
                messageNumber++;
            ...
            setTimeout(function () {
                $('.typing-animation').removeClass('hidden');
            }, 2000);
        } else {
            clearInterval(messageInterval);
